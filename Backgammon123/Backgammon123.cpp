...
//461
int validateMove(board_t &board, int move[2], char player, int dice[4], int test , int forcehit = 0) {
	...
	for (int i = 0; i < 4; i++) {
		if (dice[i] > 0) {
			if (player == 'R')
				...
				move[1] = 24 + d * dice[i];
			if (validateMove(board, move, player, dice, 1, 0) == 1) {
				return 1;
			}
		}
	}
...
int hitpossible(board_t board, int m1, int dice[4], char player) {
	int move[2];
	int dir = 1;
	if (player == 'W')
		dir = -1;
	...
	if (board.board[move[0]] == player) {
		for (int i = 0; i < 4; i++) {
			move[1] = m1 + dir * dice[i];
			if (move[1] >= 0 && move[1] <= 23)
				if (validatePos(board, move, player) == HIT)
					return 1;
		}
	}
	return 0;
...
int forcehitCheck(board_t board, int dice[4], char player) {
	int forcehit = 0;
	for (int i = 0; i < 24; i++) {
		if (hitpossible(board, i, dice, player))
			forcehit = 1;
	}
	if (player == 'R' && board.board_numbers[REDBAR] > 0)
		forcehit = 0;
...
void moveAvailable(board_t &board, char pawn_to_move[INPUTL], char pos_to_move[INPUTL], char player, int dice[4], char* fname) {
	int temp;
	int forcehit = forcehitCheck(board, dice, player);
	...
	if (checkBar(board, player, pawn_to_move)) {
		...
		if (validateMove(board, move, player, dice, 0, forcehit))
